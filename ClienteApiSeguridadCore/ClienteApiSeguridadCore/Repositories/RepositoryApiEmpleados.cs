...
namespace ClienteApiSeguridadCore.Repositories
{
    public class RepositoryApiEmpleados: IRepositoryApiEmpleados
    {
        ...
        {
            this.uriapi = "https://localhost:51552/";
        ...
        {
            List<Empleado> empleados = await this.CallApi<List<Empleado>>("api/Empleados", token);
            return empleados;
        }

        public async Task<List<Empleado>> GetEmpleadosSubordinados(string token)
        {
            List<Empleado> empleados = await this.CallApi<List<Empleado>>("api/Empleados/GetSubordinados", token);
            return empleados;
        }

        public async Task<Empleado> PerfilEmpleado(string token)
        {
            Empleado empleado = await this.CallApi<Empleado>("api/Empleados/PerfilEmpleado", token);
            return empleado;
        }
